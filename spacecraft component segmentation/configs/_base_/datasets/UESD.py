...
train_dataloader = dict(  # 训练数据加载器(dataloader)的配置
    batch_size=8,  # 每一个GPU的batch size大小
    ...
    dataset=dict(  # 训练数据集配置
        ...
        data_prefix=dict(
            img_path='image/train', seg_map_path='annotation/train'),  # 训练数据的前缀
...
val_dataloader = dict(
    ...
    dataset=dict(  # 测试数据集配置
        ...
        data_root=data_root,  # 数据集的根目录
        data_prefix=dict(
            img_path='image/val', seg_map_path='annotation/val'),  # 测试数据的前缀
